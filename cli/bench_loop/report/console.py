...
def render_run_summary(run: BenchmarkRun) -> None:
    ...
    if run.stack.engine:
        _info_row("ENGINE", f"{run.stack.engine} ({run.provider})")
    else:
        _info_row("HARNESS", f"{run.harness} ({run.provider})")
    if run.stack.quantization:
        _info_row("QUANT", run.stack.quantization)
    if run.stack.context_length:
        _info_row("CONTEXT", f"{run.stack.context_length:,}")
    ...
    rows = [
        ...
        ("MEMORY", (run.suites.get("memory").score if run.suites.get("memory") else 0.0)),
        ("STABILITY", run.reliability_score),
        ("QUALITY RET.", run.quality_score),
    ...
    console.print(
        f"  [{BOLD_ACCENT}]LOCAL SCORE   [/{BOLD_ACCENT}][{DIM}]│[/{DIM}]  [{style}]{overall:.1f}[/{style}]  {badge}"
    )
    if run.can_it_run:
        console.print(f"  [{ACCENT}]CAN IT RUN?   [/{ACCENT}][{DIM}]│[/{DIM}]  {run.can_it_run}")
