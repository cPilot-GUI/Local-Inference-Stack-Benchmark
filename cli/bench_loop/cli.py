...
@click.option("--engine", default=None, help="Inference engine label (e.g. cPilot, Ollama, LM Studio, llama.cpp, MLX).")
@click.option("--engine-version", default="", help="Inference engine version stamped on the run.")
@click.option("--quantization", default=None, help="Quantization label (e.g. Q4_K_M, Q5_K_M, Q8, IQ4).")
@click.option("--context-length", default=None, type=int, help="Primary context length for stack grouping.")
@click.option(
    "--suites",
...
def export(output: str | None, include_all: bool) -> None:
    ...
    """
    REQUIRED_FULL = {"speed", "memory", "stability", "quality_retention"}
    ...
    for run_dir in sorted(RUNS_DIR.iterdir(), reverse=True):
        ...
        memory_metrics = data.get("memory_metrics") or {}
        stability_metrics = data.get("stability_metrics") or {}
        quality_retention_metrics = data.get("quality_retention_metrics") or {}
        row = {
            "id": run_dir.name,
            "timestamp": data.get("timestamp", ""),
            "model": model_id,
            "model_family": stack.get("model_family") or (data.get("model") or {}).get("family", ""),
            "parameter_size": stack.get("parameter_size") or (data.get("model") or {}).get("parameter_count", ""),
            "quantization": stack.get("quantization") or (data.get("model") or {}).get("quantization", ""),
            "engine": stack.get("engine") or data.get("provider", ""),
            "engine_version": stack.get("engine_version", ""),
            "stack": stack,
            "harness": data.get("harness", "raw"),
            "provider": data.get("provider", ""),
            "context_length": stack.get("context_length", 0),
            "runtime_config": stack.get("runtime_config", {}),
            "hardware_class": stack.get("hardware_class", ""),
            "can_it_run": data.get("can_it_run", ""),
            "is_stack_benchmark": bool(data.get("is_stack_benchmark")),
            "legacy_model_run": bool(data.get("legacy_model_run", not data.get("is_stack_benchmark"))),
            ...
            "memory_score": (suite_map.get("memory") or {}).get("score", 0),
            "reliability_score": data.get("reliability_score", 0),
            "quality_retention_score": quality_retention_metrics.get("score", data.get("quality_score", 0)),
            "generation_tok_per_sec": (data.get("speed_metrics") or {}).get("generation_tok_per_sec", 0),
            "prompt_eval_tok_per_sec": (data.get("speed_metrics") or {}).get("prompt_eval_tok_per_sec", 0),
            "ttft_ms": (data.get("speed_metrics") or {}).get("ttft_ms", 0),
            "peak_ram_gb": memory_metrics.get("peak_ram_gb", 0),
            "peak_vram_gb": memory_metrics.get("peak_vram_gb", 0),
            "oom_rate": memory_metrics.get("oom_rate", 0),
            "stability_success_rate": stability_metrics.get("success_rate", data.get("reliability_score", 0)),
        ...
        # Keep best run per stack tuple.
        key = "::".join([
            model_id,
            str(row["quantization"]),
            str(row["engine"]),
            str(row["hardware_class"] or row["machine"]),
